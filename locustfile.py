...
GET_MAP_MAX_WIDTH = int(os.getenv("GET_MAP_MAX_WIDTH", "512"))
GET_MAP_MAX_HEIGHT = int(os.getenv("GET_MAP_MAX_HEIGHT", "512"))
...
def getCommonWMSRequestParams(wmsversion: str):
    commonparams = {"service": "wms", "version": wmsversion}
...
def getPeriodTimedelta(period: str):
    ...
    delta = {"years": 0, "months": 0, "days": 0, "hours": 0, "minutes": 0, "seconds": 0}
    ...
    if nonZeroTotalPeriod is False:
        raise Exception(
            "Invalid time period: '{}'. Period has zero length.".format(period)
        )
    ...
    for definition in definitions:
        if not "/" in definition:
            ...
            try:
                ...
                raise Exception(
                    "Error parsing the step in time definition: '{}'".format(definition)
                )
            while start <= end:
...
def getRandomLegendUrlRequest(allLayers: dict, wmsversion: str):
    ...
    getLegendGraphicRequest = {"url": url.split("?")[0]}
    ...
    if (
        not "height" in getLegendGraphicRequest.keys()
        or getLegendGraphicRequest["height"] == ""
    ):
        getLegendGraphicRequest["height"] = 20
    if (
        not "width" in getLegendGraphicRequest.keys()
        or getLegendGraphicRequest["width"] == ""
    ):
...
def sendGetCapabilitiesRequest(client: locustclients.HttpSession, wmsversion: str):
    ...
    with client.request(
        ...
        try:
            ...
            vprint(
                "Request failed (Failed to parse GetCapabilities XML): {}".format(url)
            )
        ...
        try:
            ...
            parsed_layers = getAllLayers(
                capabilities=capabilities, wmsversion=wmsversion
            )
            return parsed_layers
        except Exception as ex:
            vprint(
                "Request failed (Failed to extract layers from GetCapabilities XML): {}".format(
                    url
                )
            )
            response.failure(
                "Failed to extract layers from GetCapabilities XML version '{0}'. Error: {1}".format(
                    wmsversion, ex
                )
            )
            return
...
def sendGetMapRequest(client: locustclients.HttpSession, params: dict, wmsversion: str):
    with client.request(
        ...
        "",
        params=params,
        name="WMS-{}-GetMap".format(wmsversion),
        catch_response=True,
    ) as response:
        if response.url is None:
            response.failure("Request failed. No response from server.")
        ...
        if response.status_code == 200:
            if response.headers["content-type"] != "image/png":
                errormsg = "Expected format 'image/png' but got '{}' instead\n Request URL: {}\n Request params: '{}'\nResponse: '{}'\n".format(
                    response.headers["content-type"], url, params, response.text
                )
                vprint("Request failed:\n{}".format(errormsg))
                response.failure(errormsg)
            else:
...
def sendGetLegendGraphicRequest(
    client: locustclients.HttpSession, params: dict, wmsversion: str
):
    requestUrl = ""
    if "url" in params:
...
class WebsiteTasks(TaskSet):
    ...
    def get_capa(self):
        ...
        all_layers = sendGetCapabilitiesRequest(
            client=self.client, wmsversion=wmsversion
        )
    ...
    def get_legend_graphic(self):
        ...
        getLegendGraphicRequest = getRandomLegendUrlRequest(
            self.allLayers[wmsversion], wmsversion=wmsversion
        )

        if not getLegendGraphicRequest is None:
            sendGetLegendGraphicRequest(
                client=self.client,
                params=getLegendGraphicRequest,
    ...
    def get_map(self):
        ...
        if not wmsversion in self.allLayers.keys():
            # Get Capabilities document not (yet) processed
            return
        getMapRequest = getRandomGetMapRequest(
            self.allLayers[wmsversion], wmsversion=wmsversion
        )
        sendGetMapRequest(
            client=self.client, params=getMapRequest, wmsversion=wmsversion
        )
...
if __name__ == "__main__":
    ...
    for provider in ["dwd", "hh"]:
        ...
        for wmsversion in WMS_SUPPORTED_VERSIONS:
            capabilities = None
            with open(
                "{}/testdata/getcapabilities_{}_{}.xml".format(
                    currentPath, wmsversion, provider
                ),
                "r",
                1024,
                "utf-8",
            ...
            allLayers = {
                wmsversion: getAllLayers(
                    capabilities=capabilities, wmsversion=wmsversion
                )
            }
            getMapRequest = getRandomGetMapRequest(
                allLayers[wmsversion], wmsversion=wmsversion
            )
            print(
                "{}-WMS-{}-GetMap-Request: \n{}\n".format(
                    provider, wmsversion, getMapRequest
                )
            )
            getLegendGraphicRequest = getRandomGetLegendGraphicRequest(
                allLayers[wmsversion], wmsversion=wmsversion
            )
            print(
                "{}-WMS-{}-GetLegendGraphic-Request: \n{}\n".format(
                    provider, wmsversion, getLegendGraphicRequest
                )
            )
            getLegendURLRequest = getRandomLegendUrlRequest(
                allLayers[wmsversion], wmsversion=wmsversion
            )
            print(
                "{}-WMS-{}-GetLegendGraphic-Request-From_LegendURL: \n{}\n".format(
                    provider, wmsversion, getLegendURLRequest
                )
            )
